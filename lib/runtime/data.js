...
function getUserCode(userId) {
    if(cachedUserCode[userId]) {
        return q.when(cachedUserCode[userId].doc);
    }
    return db['users.code'].findOne({$and: [{user: userId}, {activeWorld: true}]})
        .then(doc => {
            if(doc && !cachedUserCode[userId]) {
                let sub = common.storage.pubsub.subscribe(`user:${userId}/code`, () => {
                    delete cachedUserCode[userId];
                    sub();
                });
                cachedUserCode[userId] = {doc};
            }
            return doc;
        });
}
...
function getRoomStatusData() {
    if(Date.now() > roomStatusDataCache.expireTimestamp) {
        roomStatusDataCache.expireTimestamp = Date.now() + 60*1000;
        return env.get(env.keys.ROOM_STATUS_DATA).then(data => {
            roomStatusDataCache.data = JSON.parse(data);
            return roomStatusDataCache.data;
        });
    }
    return q.when(roomStatusDataCache.data)
}
